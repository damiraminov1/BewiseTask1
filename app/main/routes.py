...
def get_quizzes_route():
    ...
    while True:
        try:
            ...
            for question_data in questions:
                ...
                if db.session.query(QuizzQuestion).filter(
                        QuizzQuestion.id == question_data["id"]).one_or_none():
                    raise ValueError("Database already has this question.")
            break
        except ConnectionError or ValueError:
            continue

    last_question = db.session.query(QuizzQuestion).order_by(desc(QuizzQuestion.local_uploaded_at)).first()
    ...
    for question_data in questions:
        ...
        db.session.add_all((question, category,))
        db.session.commit()
